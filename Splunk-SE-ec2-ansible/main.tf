...
resource "local_file" "ansible_inventory" {
  ...
  content = <<EOF
[splunk]
${replace(var.instance_name, " ", "-")} ansible_host=${aws_instance.splunk_server.public_ip} ansible_user=ec2-user ansible_ssh_private_key_file=./keys/${local.final_key_name}.pem
EOF
}
