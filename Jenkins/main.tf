...
resource "local_file" "ansible_inventory" {
  content  = <<-EOT
  [jenkins]
  ${aws_instance.jenkins.public_ip} ansible_user=ec2-user ansible_ssh_private_key_file=./keys/${local.key_name_final}.pem ansible_python_interpreter=/usr/bin/python3
  EOT
