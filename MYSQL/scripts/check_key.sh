...
if ! acquire_lock; then
  output_json "false" "false" "Failed to acquire lock for key creation"
  exit 1
fi
...
if [ $AWS_EXISTS -eq 0 ] && [ $S3_EXISTS -ne 0 ]; then
  release_lock
  output_json "false" "true" "Key exists in AWS but not in S3"
  exit 1
fi
...
if [ $AWS_EXISTS -eq 0 ] && [ $S3_EXISTS -eq 0 ]; then
  ...
  mkdir -p "$LOCAL_KEY_DIR" || {
    release_lock
    output_json "true" "true" "Failed to create local directory"
    exit 1
  }
  ...
  if ! aws s3 cp "s3://${BUCKET}/clients/${USERMAIL}/keys/${CLEAN_KEY_NAME}.pem" "${LOCAL_KEY_DIR}/${CLEAN_KEY_NAME}.pem" --region "$AWS_REGION" >/dev/null 2>&1; then
    release_lock
    output_json "true" "true" "Failed to download key from S3"
    exit 1
  fi
  
  chmod 0400 "${LOCAL_KEY_DIR}/${CLEAN_KEY_NAME}.pem" || {
    release_lock
    output_json "true" "true" "Failed to set key permissions"
    exit 1
  }
